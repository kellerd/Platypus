 ...
 * Professor: Sv. Ranev
 * PURPOSE:
 *    Main driver program for testing the symbol table.
 * Function List:
 *
 */
 ...
 *  usage: platytt source.pls [-st size] [-sts]
 */
...
/* __DATE__, __TIME__, __LINE__, __FILE__ are predefined preprocessor macros*/
       ...
       err_printf("%s%s%s","Usage: ", "platytt", "  source_file_name [-st size][-sts]");
        exit(EXIT_FAILURE);
	}	
...
/* print Symbol Table */
       if(sym_table.st_size){
		...
		#ifdef DEBUG
      	/* Debug mode in this, change the types */
			st_update_type(sym_table, st_lookup(sym_table, "iNum"), 'I');
         st_update_type(sym_table, st_lookup(sym_table, "INum"), 'F');
         st_update_type(sym_table, st_lookup(sym_table, "lNum"), 'F');
         st_update_type(sym_table, st_lookup(sym_table, "LNum"), 'I');
         st_update_type(sym_table, st_lookup(sym_table, "iSum$"), 'I');
         st_update_type(sym_table, st_lookup(sym_table, "nNum$"), 'S');
         st_update_type(sym_table, st_lookup(sym_table, "nNum"), 'S');
         st_update_type(sym_table, st_lookup(sym_table, "NNum"), 'S');
         /* Print */
			st_print(sym_table);

         /* Change types again (mostly errors happen)*/
         st_update_type(sym_table, st_lookup(sym_table, "iNum"), 'F');
         st_update_type(sym_table, st_lookup(sym_table, "INum"), 'I');
         st_update_type(sym_table, st_lookup(sym_table, "lNum"), 'I');
         st_update_type(sym_table, st_lookup(sym_table, "LNum"), 'F');
         st_update_type(sym_table, st_lookup(sym_table, "iSum$"), 'F');
         st_update_type(sym_table, st_lookup(sym_table, "nNum$"), 'I');
         st_update_type(sym_table, st_lookup(sym_table, "nNum"), 'F');
         st_update_type(sym_table, st_lookup(sym_table, "NNum"), 'I');
         /* Print */
			st_print(sym_table);
      #endif
