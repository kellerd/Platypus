...
let removeIfFound (globalVars: IDictionary<string,obj>)  key = 
...
let evalProgram (globalVars: IDictionary<string,obj>) (program:Program) =
    ...
    let evalAExpression expr = 
        let rec (|IsFloat|_|) expr = 
            let rec mapFloatExpr = function
                ...
                | v -> v
            match expr with
            | Arithmetic (IsFloat(_),_,_) -> Some expr
            | Arithmetic (_,_,IsFloat(_)) -> Some expr
            | Arithmetic(_) -> None
    ...
    let evalConditional = 
        ...
        and evalRelational = function
            | Str (primaryString , op , primaryString2) -> 
                 (evalPrimaryString primaryString,op,evalPrimaryString primaryString2) |> comp 
            | Arith (primaryNum , op , primaryNum2) ->
               (evalPrimaryNum primaryNum,op,evalPrimaryNum primaryNum2) |> compA 
    ...
    let rec evalStatement = function
        ...
        | Assign(ArithmeticAssignment(var,expression)) -> evalAExpression expression |> (function | Short(v) -> v |> setVar (AVID var)
                                                                                                  | Single(v) -> v |> setVar (AVID var))
        | Assign(StringAssignment(var,expression)) -> evalSExpression expression |> setVar (SVID var)
