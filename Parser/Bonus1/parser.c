...
{
	unsigned char match = lookahead.t_code == token_code;
   if (match) {
	   switch(token_code) {
      	...
      	case REL_OP_T:
         	...
         	match = lookahead.attribute.get_int == attribute_code;
      }
   }
   ...
   if (match)
   {
      ...
      if (plListToInsert != NULL)
		stack_push(plListToInsert, lookahead);
...
void assignment_statement(void) {
   ...
   Start = sem_analisys.plnHead->plnNextPointer;
   post_fix(&re_arrange);
...
void assignment_expression(void) {
   ...
   switch(lookahead.t_code) {
   	case AVID_T:
         ...
         match(ASS_OP_T, NO_ATTR, &sem_analisys);
         arithmetic_expression();
		   gen_imcode("PLATY: Assignment expression (arithmetic) parsed");
         break;
      case SVID_T:
      	match(SVID_T, NO_ATTR, NULL);
...
void input_statement(void){
   match(KW_T,READ, NULL);match(LPR_T,NO_ATTR, NULL);variable_list();
   match(RPR_T,NO_ATTR, NULL); match(EOS_T,NO_ATTR, NULL);
...
void output_statement(void){
   match(KW_T,WRITE, NULL);match(LPR_T,NO_ATTR, NULL);output_line();
   match(RPR_T,NO_ATTR, NULL); match(EOS_T,NO_ATTR, NULL);
...
void if_statement (void) {
	match(KW_T, IF, NULL);
   match(LPR_T,NO_ATTR, NULL); conditional_expression(); match(RPR_T,NO_ATTR, NULL);
...
void iteration_statement(void) {
   ...
   match(LPR_T, NO_ATTR, NULL);
   	opt_assignment_expression(); match(COM_T, NO_ATTR, NULL);
   	conditional_expression(); match(COM_T, NO_ATTR, NULL);
      opt_assignment_expression();
...
void relational_operator(void) {
	...
	if (lookahead.t_code == REL_OP_T) {
		switch(lookahead.attribute.rel_op) {
			case GT:
		   	match(REL_OP_T, GT, NULL);
         	break;
         case LT:
	   		match(REL_OP_T, LT, NULL);
            break;
   		case NE:
		   	match(REL_OP_T, NE, NULL);
            break;
   		case EQ:
		   	match(REL_OP_T, EQ, NULL);
            break;
         default:
         	syn_printe();
            break;
...
void primary_a_relational_expression(void) {
	switch(lookahead.t_code) {
   	case IL_T:
	   	match(IL_T, NO_ATTR, NULL);
         break;
      case FPL_T:
	   	match(FPL_T, NO_ATTR, NULL);
         break;
   	case AVID_T:
	   	match(AVID_T, NO_ATTR, NULL);
         break;
