...
int st_vrinsert(STD sym_table,char *lexeme, int line)
{
   ...
   #ifdef DEBUG
   	printf("Inserted: %s Value: %d Type: %c\n", sym_table.pstvrs[sym_table.st_offset].plex, sym_table.pstvrs[sym_table.st_offset].i_value.int_val, get_type(sym_table, sym_table.st_offset));
   #endif
	return sym_table.st_offset;
}
