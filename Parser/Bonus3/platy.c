...
extern List sem_analisys;
extern Stack re_arrange;
...
/*check for correct arrguments - source file name */
	if (argc == 2) {
      ...
      scanf("%s", FileName);
       ...
       err_printf("%s%s%s","Usage: ", "platy", "  source_file_name");
        exit(EXIT_FAILURE);
	}
...
/* load source file into input buffer  */
	...
	if ((fi = fopen(FileName,"r")) == NULL){
		err_printf("%s%s%s%s",argv[0],": ", "Cannot open file: ",FileName);
		exit (EXIT_FAILURE);
	}
   printf("\n\n-== The Playful Predictive PLATYPUS Interpreter ==-\n\n");
...
/*find the size of the file  */
    if (loadsize == LOAD_FAIL){
     printf("The input file %s %s\n", FileName,"is not completely loaded.");
...
void garbage_collect(void){
  printf("\n\nEnd of Program...\n\nInterpreter brought to you by Danny Keller\n");
	b_destroy(sc_buf);
	b_destroy(str_table);
	st_destroy(sym_table);
   stack_destroy(&re_arrange);
   list_destroy(&sem_analisys);
}
